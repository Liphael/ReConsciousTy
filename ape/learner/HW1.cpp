...
/*
- 数据的定义语法：
数据类型 变量名 = 初始值;例：int a = 10;
C++中，对数据类型进行定义的意义在于，它告诉编译器如何解释和存储该数据，给变量分配明确的内存空间，并确保数据操作的正确性和效率。
数据类型所占内存空间大小可以用 sizeof(数据类型名/变量名) 来查看。

常见类型：
short 短整型，占用2字节，范围约为-32,768到32,767（-2的15次方~2的15次方-1）
int 整型，占用4字节，范围约为-2,147,483,648到2,147,483,647（）
long 长整型，占用4字节，范围约为-2,147,483,648到2,147,483,647（-2的31次方~2的31次方-1）
long long 长长整型，占用8字节，范围约为-9,223,372,036,854,775,808到9,223,372,036,854,775,807（-2的63次方~2的63次方-1）


float 单精度浮点型，占用4字节，范围约为1.2E-38到3.4E+38，精度约为7位十进制数
double 双精度浮点型，占用8字节，范围约为2.3E-308到1.7E+308，精度约为15位十进制数
*/